# @swifty.js/react-signal

A lightweight TypeScript frontend framework for single-page applications and micro-frontend scenarios.

## Overview

swifty-react-signal is a functional-first framework built on **signals reactivity**
...
```tsx
import { render, useSignal } from "@swifty.js/react-signal";
...
```bash
# pnpm
pnpm add @swifty.js/react-signal

# npm
npm install @swifty.js/react-signal

# yarn
yarn add @swifty.js/react-signal
```
...
Add `"@swifty.js/react-signal/client"` to `types` (or a triple-slash reference) for
`*.css` module declarations and the HMR globals.

### Bundler plugin

Install the bundler plugin matching your build tool. The Vite plugin also
defaults `oxc.jsx = { runtime: "automatic", importSource: "@swifty.js/react-signal" }`
...
> rebuilding swifty-react-signal run `vite --force` (or delete `node_modules/.vite`)
...
```tsx
// src/views/home.tsx
import { useSignal } from "@swifty.js/react-signal";
...
```ts
// src/main.ts
import { render } from "@swifty.js/react-signal";
...
```tsx
import { useSignal, useComputed, useEffect } from "@swifty.js/react-signal";
...
```ts
import { render, unmount } from "@swifty.js/react-signal";
...
```ts
import { createRouter } from "@swifty.js/react-signal";
...
```tsx
import { useRouter } from "@swifty.js/react-signal";
...
```tsx
import { useUrlState } from "@swifty.js/react-signal";
...
```tsx
import { createStore, computed } from "@swifty.js/react-signal";
...
matches render synchronously. Share `@swifty.js/react-signal` as a singleton in the MF
...
| HMR       | `hotSwapByComponent` (also on `globalThis.__swifty_hmr__`)                                     |
...
| Import                                    | Description                                                                |
| ----------------------------------------- | -------------------------------------------------------------------------- |
| `@swifty.js/react-signal`                 | Main runtime API (`render`, hooks, `raw`, `Fragment`, ...)                 |
| `@swifty.js/react-signal/jsx-runtime`     | JSX automatic runtime (`jsx`, `jsxs`, `Fragment`, `raw`, JSX types)        |
| `@swifty.js/react-signal/jsx-dev-runtime` | JSX automatic dev runtime (`jsxDEV`)                                       |
| `@swifty.js/react-signal/vite`            | Vite plugin (`swiftyReactSignalPlugin`)                                    |
| `@swifty.js/react-signal/webpack`         | Webpack integration (`SwiftyReactSignalPlugin`, `swiftyReactSignalLoader`) |
| `@swifty.js/react-signal/client`          | Ambient types (`*.css` modules, HMR globals)                               |
