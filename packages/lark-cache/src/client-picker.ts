...
const DEFAULT_SVC_NAME = "lark_cache";
...
export class ClientPicker implements PeerPicker {
  ...
  printPeers(): void {
    console.log("[LarkCache] Discovered peers:");
  ...
  private onPeerDiscovered(addr: string): void {
    ...
    this.addPeer(addr);
    console.log(`[LarkCache] New service discovered at ${addr}`);
  }
  ...
  private onPeerRemoved(addr: string): void {
    ...
    if (client) {
      ...
      console.log(`[LarkCache] Service removed at ${addr}`);
    }
  }
  ...
  private addPeer(addr: string): void {
    ...
    console.log(`[LarkCache] Successfully created client for ${addr}`);
  }
}
