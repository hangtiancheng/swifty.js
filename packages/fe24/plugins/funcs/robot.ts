...
export const robotQueryFn: Connect.NextHandleFunction = (req, res) => {
  ...
  if (id) {
    resData = resData.filter(
      (item) => item.id === Number.parseInt(id as string),
    );
    res.end(
      JSON.stringify({
        code: 200,
  ...
  if (address) {
    resData = resData.filter((item) =>
      item.address?.includes(address as string),
    );
  }

  if (state && Number.parseInt(state as string) > 0) {
    resData = resData.filter(
      (item) => item.state === Number.parseInt(state as string),
    );
  }
  ...
  if (Number.parseInt(pageSize as string) > 0) {
    const start =
      (Number.parseInt(pageNum as string) - 1) *
      Number.parseInt(pageSize as string);
...
export const robotAddFn: Connect.NextHandleFunction = (req, res) => {
  ...
  robotList.unshift({
    ...
    lng: Number.parseFloat(
      (lng as string) ?? (Math.random() * (30 - 28) + 28).toFixed(6),
    ),
    // 如果未传递 lat, 则随机生成一个
    lat: Number.parseFloat(
      (lat as string) ?? (Math.random() * (120 - 118) + 118).toFixed(6),
    ),
  });
