...
- **Critical: globalThis pollution breaking other extensions' command registration**
  ...
  This was originally added to satisfy a transitive dependency (`@lark.js/mvc`) that expected a browser environment. However, `@lark.js/mvc` was never actually imported at runtime — it only appeared as a string literal in project detection logic and generated code output. The shim was entirely unnecessary.
  ...
  - Removed `@lark.js/mvc` from the `noExternal` bundling list, as it was never imported
  ...
  - Converting `findLarkRoots()` to return a `Promise`
