 ...
 *
 * Lark HTML templates use interpolation syntax like {{=variable}} inside
...
export async function disableCssValidation(): Promise<void> {
  ...
  try {
    await config.update("validate", false, vscode.ConfigurationTarget.Workspace);
    log("CSS validation disabled in workspace settings for Lark template compatibility");
  } catch {
    log("Failed to update css.validate setting (workspace may not have .vscode directory)");
  }
}
