...
import { BaseEvaluator, trimExtremes, type JudgeSample } from "@/evaluator/base-evaluator.js";
...
describe("BaseEvaluator.evaluate", () => {
  ...
  it("averages all samples when below the trimming threshold", async () => {
    ...
    const evaluation = await new StubEvaluator(client, 2).evaluate(record, task);
  ...
  it("parses fenced judge verdicts", async () => {
    ...
    const evaluation = await new StubEvaluator(client, 1).evaluate(record, task);
