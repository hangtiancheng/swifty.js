...
describe("MarkdownGenerator", () => {
	it("renders a batch report with overview, evidence, dialogues, and recommendations", () => {
		...
		expect(report).toContain("### 对话 2: 对抗型用户");
...
describe("HtmlGenerator", () => {
	...
	it("escapes HTML in dialogue content", () => {
		const generator = new HtmlGenerator({ now });
		const html = generator.generateBatch([
			makeResult("配合型用户", '<script>alert("x")</script>'),
		]);
...
describe("English reports", () => {
	it("renders English copy when the locale is en", () => {
		...
		const markdown = new MarkdownGenerator({ now }).generateBatch([
			makeResult("配合型用户", "你好"),
		]);
		expect(markdown).toContain("**Generated at:** 2026-08-25 12:30:45");
		expect(markdown).toContain("## 1. Evaluation Overview");
		expect(markdown).toContain("**Average score:** 36.0/100");
		expect(markdown).toContain("**Termination reason:** User refused");

		const html = new HtmlGenerator({ now }).generateBatch([
			makeResult("配合型用户", "你好"),
		]);
		expect(html).toContain('<html lang="en">');
		expect(html).toContain("配合型用户 — Overall Score");
	});
});
