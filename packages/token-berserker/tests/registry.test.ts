...
describe("EvaluatorRegistry.evaluateAll", () => {
	it("collects every dimension result", async () => {
		...
		const registry = new EvaluatorRegistry(
			[
				new FixedEvaluator(client, "flowCompletion", {
					score: 0.9,
					reason: "good",
				}),
				new FixedEvaluator(client, "faqAccuracy", {
					score: 0.7,
					reason: "fine",
				}),
			],
		...
		expect(results.get("flowCompletion")).toEqual({
			score: 0.9,
			reasons: ["good"],
		});
		expect(results.get("faqAccuracy")).toEqual({
			score: 0.7,
			reasons: ["fine"],
		});
	});
