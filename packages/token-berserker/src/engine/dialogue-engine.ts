...
export function parseHangupSignal(message: string): {
	content: string;
	hangup: boolean;
} {
	if (message.includes(HANGUP_SIGNAL)) {
		return {
			content: message.replaceAll(HANGUP_SIGNAL, "").trim(),
			hangup: true,
		};
	}
...
export class DialogueEngine {
	...
	async runDialogue(params: RunDialogueParams): Promise<DialogueRecord> {
		...
		let modelMessage = resolvePlaceholders(
			this.task.openingLine,
		...
		for (let round = 1; round <= maxRounds; round += 1) {
			...
			if (
				hangup ||
				(round > minRounds &&
					(await this.judgeRefusal(refusalJudge, userResponse)))
			) {
	...
	private async generateModelResponse(
		...
		if (this.task.constraints.maxChars !== undefined) {
			constraints.push(
				`Keep each reply within ${this.task.constraints.maxChars} characters`,
			);
		}
		...
		if (this.task.constraints.forbiddenPhrases.length > 0) {
			constraints.push(
				`Do not use: ${this.task.constraints.forbiddenPhrases.join(", ")}`,
			);
		}
