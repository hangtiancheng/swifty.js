# @lark.js/docs Technical Plan

## 1. Goal

`@lark.js/docs` is to `@lark.js/mvc` what VitePress is to Vue3 and Docusaurus is to React -- an out-of-the-box documentation site generator. It recursively scans a user-provided `docs/` directory, compiles every `.md` file into a Lark View, and produces a complete documentation site with navigation, sidebar, search, and code highlighting.
...
```
  ...
  +-----------------------------------+    +-------------------------------------------+
                  ...
                  v                                        |
  +-----------------------------------+    +-------------------------------------------+
  |  @lark.js/docs Build Pipeline      |    |  @lark.js/docs Runtime                     |
...
```jsonc
// packages/lark-docs/package.json
{
  "name": "@lark.js/docs",
  "version": "0.0.1",
...
Users create a `lark-docs.config.ts` (or `.js` / `.mjs`) at the project root:
...
function generateModule(
  ...
  return `
// Generated by @lark.js/docs from: ${options.filePath}
...
const LARK_DOC_SUFFIX = "?lark-docs-template";
...
export default View.extend({
  ...
  assign() {
    ...
    const docsConfig = State.get("docsConfig") as Record<string, unknown>;
    this.updater.set({
...
## 15. Boot Integration (how @lark.js/docs connects to @lark.js/mvc)

### 15.1 Build-Time: Virtual Modules

The build plugin generates two virtual modules:

**`virtual:lark-docs-site-data`** -- site metadata (title, nav, sidebar, search index):
...
**`virtual:lark-docs-routes`** -- route map + page data:
...
```ts
// User's boot.ts (or auto-generated by @lark.js/docs)
import { Framework, registerViewClass, State } from "@lark.js/mvc";
import { docsConfig } from "virtual:lark-docs-site-data";
import { routes, pages } from "virtual:lark-docs-routes";
import DocLayout from "@lark.js/docs/theme/doc-layout";
import Sidebar from "@lark.js/docs/theme/sidebar";
import Content from "@lark.js/docs/theme/content";
import Toc from "@lark.js/docs/theme/toc";
import Search from "@lark.js/docs/theme/search";
...
For an out-of-the-box experience, `@lark.js/docs` can provide a CLI that generates the boot file automatically:

```bash
# Development
npx lark-docs dev --docs ./docs --port 3000

# Build
npx lark-docs build --docs ./docs --outDir ./dist

# Preview built site
npx lark-docs preview --outDir ./dist --port 4000
```

The CLI:

1. Reads `lark-docs.config.ts` from the project root
...
```
my-project/
  lark-docs.config.ts       # @lark.js/docs configuration
...
```ts
// packages/lark-docs/vitest.config.ts
...
{
  "scripts": {
    "test:doc": "pnpm --filter @lark.js/docs test"
  }
}
```
...
Since styling is not user-customizable, `@lark.js/docs` bundles its own Tailwind + DaisyUI setup:
...
export default {
  content: [require.resolve("@lark.js/docs/dist/theme/**/*.html")],
  plugins: [daisyui],
  daisyui: {
    themes: ["light", "dark"],
  },
