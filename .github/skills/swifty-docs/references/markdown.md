...
- Renders to `<div swifty-docs-toc></div>`; at runtime `ContentRenderer` mounts an inline React `Toc` component into every placeholder.
...
- Internal links (href starting `/` or `#`) get `swifty-docs-nav="true"`; `ContentRenderer` intercepts clicks for SPA navigation (react-iso route) and smooth-scrolls anchor links.
